"""Methods for interfacing with a PlayStation 5 Console."""
import asyncio
import logging
import socket
import time
...
from .connection import DEFAULT_LOGIN_DELAY, AsyncConnection, LegacyConnection
...
class Ps5Base():
    """The PS5 base object. Should not be initialized directly.

    :param host: The host PS5 IP address
    ...
    def get_status(self) -> dict:
        ...
        try:
            ...
            _LOGGER.debug("PS5 @ %s status timed out", self.host)
            self.status = None
            return self.status
    ...
    def is_running(self) -> bool:
        """Return True if the PS5 is running."""
        if self.status is not None:
    ...
    def is_standby(self) -> bool:
        """Return True if the PS5 is in standby."""
        if self.status is not None:
    ...
    def is_available(self) -> bool:
        """Return True if the PS5 is available."""
        if self.status is not None:
            return True
        return False
    ...
    def connected(self) -> bool:
        """Return True if connected to PS5."""
...
class Ps5Legacy(Ps5Base):
    """Legacy PS5 Class. Sync Version.
    ...
    def open(self):
        """Open a connection to the PS5."""
        self.get_status()
        if not self.is_running:
            raise NotReady("PS5 is not On")
    ...
    def close(self):
        """Close the connection to the PS5."""
        self.connection.disconnect()
        self._connected = False
        self.loggedin = False
        self.msg_sending = False
        _LOGGER.debug("Disconnecting from PS5 @ %s", self.host)
        return True
    ...
    def login(self, pin: Optional[str] = '') -> bool:
        ...
        if not is_login:
            self.close()
            raise LoginFailed("PS5 Refused Connection")
        self.loggedin = True
        return is_login
    ...
    def remote_control(
        ...
        :param button_name: Button to send to PS5.
    ...
    def send_status(self) -> bool:
        """Send connection status ack to PS5."""
        if self._connected and self.loggedin:
            self.connection.send_status()
            return True
        _LOGGER.error("PS5 is not connected")
        return False


class Ps5Async(Ps5Base):
    """Async Version of Ps5 Class.
    ...
    def set_protocol(self, ddp_protocol: DDPProtocol):
        ...
        :param ddp_protocol: :class: `psremoteplay.ddp.DDPProtocol`
        """
    ...
    async def close(self):
        """Close Connection."""
        self._close()
        _LOGGER.debug("Closing PS5 TCP connection")
    ...
    async def async_connect(self, auto_login: Optional[bool] = True):
        ...
        if not self._connected:
            ...
            if not self.is_available:
                raise NotReady(
                    "PS5 is not available or powered off. Check connection.")
            if not self._power_off:  # If not powering off
                if self.is_standby:
                    raise NotReady("PS5 is not On")
                try:
                    _LOGGER.debug("Attempting connection with PS5")
                    self._prepare_connection()
                    tcp_transport, tcp_protocol =\
                        await self.connection.async_connect(self)
                except (OSError, ConnectionRefusedError):
                    _LOGGER.info("PS5 Refused Connection")
                    self._connected = False
                    self.loggedin = False
                else:
