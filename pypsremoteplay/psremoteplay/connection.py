# -*- coding: utf-8 -*-
"""TCP Connection Handling for PS5."""
...
class BaseConnection():
    ...
    Represents a connection to a PS5 Device.
    :param ps5: PS5 object to attach to.
    :param credential: PSN credentials to use.
    :param port: Remote port to connect to.
    """

    def __init__(
            self, ps5, credential: Optional[str] = None,
...
class LegacyConnection(BaseConnection):
    """Legacy Connection for Legacy PS5 object."""
...
class AsyncConnection(BaseConnection):
    ...
    async def async_connect(self, ps5):
        """Create asyncio TCP connection.

        :param ps5: :class: PS5Async Object to attach to.
        """
        ...
        connect = loop.create_connection(
            lambda: TCPProtocol(ps5, loop), sock=sock)
...
class TCPProtocol(asyncio.Protocol):
    ...
    def __init__(self, ps5, loop):
        ...
        self.transport = None
        self.connection = ps5.connection
        self.task = None
    ...
    def connection_made(self, transport: asyncio.Transport):
        ...
        self.ps5._connected = True  # noqa: pylint: disable=protected-access
        _LOGGER.debug("PS5 Transport Connected @ %s", self.ps5.host)

        if self.ps5.task_queue is not None:
            valid_initial_tasks = {'start_title': self.start_title,
                                   "remote_control": self.remote_control}
            args = None
            initial_task = self.ps5.task_queue
            self.ps5.task_queue = None

            if self.ps5._power_on:  # noqa: pylint: disable=protected-access
                args = initial_task[1:]
        ...
        #  Schedule to close after some time as PS5 doesn't respond.
    ...
    def connection_lost(self, exc: Exception):
        ...
        :param exc: Exception
        """
        _LOGGER.debug("Transport @ %s is disconnected", self.ps5.host)
        if self._hb_handler is not None:
            self._hb_handler.cancel()
        self.ps5._closed()  # noqa: pylint: disable=protected-access
        self.ps5 = None
    ...
    async def login(
        ...
        if not self.task == 'login':  # Only schedule one login task.
            ...
            msg = _get_login_request(
                self.ps5.credential, self.ps5.device_name, pin)
    ...
    async def _check_heartbeat(self):
        ...
        if hb_delta is not None:
            ...
            if hb_delta > self.heartbeat_timeout and self.ps5 is not None:
                _LOGGER.warning(
                    "Timed out waiting for PS5 heartbeat status; Closing...")
                self.ps5._close()  # noqa: pylint: disable=protected-access
    ...
    def _timeout_close(self):
        ...
        if time.time() - self._last_activity > self.connection_timeout:
            _LOGGER.debug("Max login time exceeded. Closing PS5 TCP connection")
            self.ps5._close()  # noqa: pylint: disable=protected-access
        else:
            self.loop.call_later(
                self.connection_timeout,
