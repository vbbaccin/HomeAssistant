...
from psremoteplay.errors import NotReady, PSDataIncomplete
from psremoteplay.media_art import TYPE_APP as PS_TYPE_APP
import psremoteplay.ps5 as pyps5
...
from homeassistant.components.ps5 import format_unique_id, load_games, save_games
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for device in config.data["devices"]:
        ...
        ps5 = pyps5.Ps5Async(host, creds, device_name=DEFAULT_ALIAS)
        device_list.append(PS5Device(config, name, host, region, ps5, creds))
    async_add_entities(device_list, update_before_add=True)


class PS5Device(MediaPlayerEntity):
    """Representation of a PS5."""

    def __init__(self, config, name, host, region, ps5, creds):
        """Initialize the ps5 device."""
    ...
    def subscribe_to_protocol(self):
        ...
        self.hass.data[PS5_DATA].protocol.add_callback(self._ps5, self.status_callback)

    @callback
    def unsubscribe_to_protocol(self):
        """Notify protocol to remove callback."""
        self.hass.data[PS5_DATA].protocol.remove_callback(
            self._ps5, self.status_callback
        )
    ...
    async def async_added_to_hass(self):
        """Subscribe PS5 events."""
        self.hass.data[PS5_DATA].devices.append(self)
        self.check_region()

    async def async_update(self):
        """Retrieve the latest data."""
        if self._ps5.ddp_protocol is not None:
            # Request Status with asyncio transport.
            self._ps5.get_status()

            # Don't attempt to connect if entity is connected or if,
            # PS5 is in standby or disconnected from LAN or powered off.
            if (
                not self._ps5.connected
                and not self._ps5.is_standby
                and self._ps5.is_available
            ):
                with suppress(NotReady):
                    await self._ps5.async_connect()

        # Try to ensure correct status is set on startup for device info.
        if self._ps5.ddp_protocol is None:
            # Use socket.socket.
            await self.hass.async_add_executor_job(self._ps5.get_status)
            if self._info is None:
                # Add entity to registry.
                await self.async_get_device_info(self._ps5.status)
            self._ps5.ddp_protocol = self.hass.data[PS5_DATA].protocol
    ...
    def state_unknown(self):
        ...
        if self._disconnected is False:
            _LOGGER.warning("PS5 could not be reached")
    ...
    async def async_get_title_data(self, title_id, name):
        ...
        media_type = None
        try:
            title = await self._ps5.async_get_ps_store_data(
    ...
    async def async_get_device_info(self, status):
        ...
        if status is None:
            ...
            self._info = {
                ...
                "model": "PlayStation 5",
                "identifiers": {(PS5_DOMAIN, status["host-id"])},
    ...
    async def async_will_remove_from_hass(self):
        ...
        if self._ps5.connected:
            await self._ps5.close()
        self.unsubscribe_to_protocol()
        self.hass.data[PS5_DATA].devices.remove(self)
    ...
    async def async_select_source(self, source):
        ...
        for title_id, data in self._games.items():
            ...
            if (
                ...
                _LOGGER.debug(
                    "Starting PS5 game %s (%s) using source %s", game, title_id, source
                )

                await self._ps5.start_title(title_id, self._media_content_id)
                return
    ...
    async def async_send_remote_control(self, command):
        """Send RC command."""
        await self._ps5.remote_control(command)
